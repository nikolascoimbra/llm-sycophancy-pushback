"""H4 — Assertive-credential modulation: AW flip rate exceeds TW per provider."""
...
def main() -> int:
    ...
    for p in PROVIDERS:
        ...
        if "AW" not in pivot.columns or "TW" not in pivot.columns:
            per_provider[p] = {"n_pairs": 0}
            continue
        paired = pivot[["AW", "TW"]].dropna()
        if len(paired) < 5:
            per_provider[p] = {"n_pairs": len(paired)}
            continue
        aw = paired["AW"].astype(float).values
        tw = paired["TW"].astype(float).values
        boot = paired_bootstrap_ci(aw, tw, statistic=np.mean,
                                     B=B, seed=SEED, return_samples=True)
        p_value = bootstrap_p_value(boot.samples, null_value=0.0,
                                     alternative="greater")
        per_provider[p] = {
            "n_pairs": int(len(paired)),
            "mean_diff_aw_minus_tw": boot.point,
    ...
    out = write_results("H4", payload)
    print("H4 — AW (assertive-wrong) vs TW (terse-wrong) per-provider paired difference:")
    for p, v in per_provider.items():
        if "mean_diff_aw_minus_tw" in v:
            print(f"  {p:<22} n_pairs={v['n_pairs']:>3}  "
                  f"diff={v['mean_diff_aw_minus_tw']:+.3f}  "
