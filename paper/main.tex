...
on factual recall, in every provider tested.} Claude Sonnet 4.6 goes from
84\% TW flip rate bare-API to 43\% with tools; GPT-5 goes from 93\% to 62\%
(and to 31\% when free-form prompting is also applied); Gemini 2.5 Pro goes
from 98\% to 19\%. The per-question, paired, within-subject contrast
clears the Holm-corrected threshold on every G-arm provider
($p < 10^{-2}$ on Anthropic, $p < 10^{-4}$ on GPT-5 and Gemini).
...
same eligible questions. Bars are the H6 matched-pair statistic
(within-subject intersection of turn-1-correct in both cells). Bootstrap
95\% CIs; per-provider paired one-sided test, every $p < 10^{-2}$.
Cell-level rates (over all turn-1-correct questions per cell, regardless
of matched availability in the other cell) are reported in
Table~\ref{tab:study2_cell} below; they are 5--15 points lower than the
matched-pair bars for Anthropic and Gemini because the matched subsamples
are small ($n_{\text{pairs}}\!=\!7$ Anthropic, $n_{\text{pairs}}\!=\!16$
Gemini) and biased toward questions where the model held confidently in
both conditions.\label{fig:F4}}
\end{figure}

\paragraph{H6 (supported).} The within-subject paired test on the matched
intersection of turn-1-correct in cells BV-TW and TV-TW:
...
The Anthropic and Gemini paired-set BV-TW rates of $1.000$ reflect a
specific small-$n$ artifact: the only questions for which we have both a
BV-TW measurement \emph{and} a successful TV-TW measurement are
disproportionately questions on which the bare-API model flipped. The cell-
level rates over all turn-1-correct questions are not 100\% --- they are
the values in Table~\ref{tab:study2_cell}.

\begin{center}
\label{tab:study2_cell}
\begin{tabular}{lrrrr}
\toprule
Provider & BV-TW flip & BF-TW flip & TV-TW flip & TF-TW flip \\
 & ($n_\text{t1ok}$) & ($n_\text{t1ok}$) & ($n_\text{t1ok}$) & ($n_\text{t1ok}$) \\
\midrule
Anthropic Sonnet 4.6 & 0.84 (68) & 0.96 (57) & 0.43 (7)\textsuperscript{*} & --- \\
GPT-5                & 0.93 (70) & 0.78 (46) & 0.62 (66)            & 0.31 (68) \\
Gemini 2.5 Pro       & 0.98 (59) & 0.73 (30) & 0.19 (32)            & 0.27 (30) \\
DeepSeek V3.2        & 0.86 (66) & 0.93 (58) & ---                  & --- \\
\bottomrule
\end{tabular}
\end{center}
\textsuperscript{*}\footnotesize{Anthropic Sonnet 4.6 TV cell terminated
at $n_{\text{TV-TW graded}}\!=\!7$ due to Anthropic Console 50-RPM rate-
limit constraints; see amendment A10. The within-subject test on those 7
matched pairs is still significant ($p\!=\!0.003$) but the cell rate
should be interpreted with caution.}

\medskip

Reading the cell-level table: bare-API + verbalized-confidence (BV-TW)
flip rates on the v1 panel are 84\%, 93\%, 98\%, 86\% --- consistent with
the Study 1 values of 90--96\% on the four-provider confirmatory panel and
with the broader literature's per-model sycophancy ranking. The dramatic
move is in the TV-TW column: same models, same questions, plus native web
search drops the flip rate to 43\%, 62\%, 19\% respectively. The
within-subject test (H6 paired bootstrap) confirms the contrast is real
on each of the three G-arm providers individually; the magnitudes reported
in the matched-pair table above are at the upper end of the cell-level
range because of the matched-subsample selection effect.

GPT-5's $n_{\text{pairs}}\!=\!66$ is large enough that its paired and cell
statistics agree (paired BV-TW $=\!0.924$, cell $=\!0.93$). Anthropic
($n_{\text{pairs}}\!=\!7$) and Gemini ($n_{\text{pairs}}\!=\!16$) show the
matched-subsample bias clearly. We report both for transparency.
